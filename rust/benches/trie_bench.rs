...
fn bench_exact_match_small(c: &mut Criterion) {
    ...
    let bytes = TrieBuilder::build_unsorted(&mut entries)
    ...
    group.bench_function("batch_10", |b| {
        let queries = ["가", "나다", "학교", "선생님", "아버지", "없음1", "없음2", "가방에", "학생", "없음3"];
        b.iter(|| {
...
fn bench_exact_match_medium(c: &mut Criterion) {
    ...
    let bytes = TrieBuilder::build(
        &entries.iter().map(|(k, v)| (k.as_str(), *v)).collect::<Vec<_>>()
    ).expect("Failed to build trie");
    let trie = Trie::new(&bytes);
...
fn bench_common_prefix_search(c: &mut Criterion) {
    ...
    group.bench_function("long_text", |b| {
        b.iter(|| {
            let results: Vec<_> = trie.common_prefix_search(black_box("아버지가방에")).collect();
            black_box(results);
        });
    });
    ...
    group.bench_function("sentence_analysis", |b| {
        ...
        b.iter(|| {
            ...
            for i in 0..text.chars().count() {
                // 각 문자 위치에서 검색
                let byte_pos = text.char_indices().nth(i).map(|(pos, _)| pos).unwrap_or(text.len());
...
fn bench_trie_build(c: &mut Criterion) {
    ...
    group.bench_function("small_sorted", |b| {
        ...
        b.iter(|| {
            let bytes = TrieBuilder::build(black_box(&entries))
    ...
    group.bench_function("small_unsorted", |b| {
        ...
        b.iter(|| {
            ...
            let bytes = TrieBuilder::build_unsorted(black_box(&mut e))
                .expect("Failed to build");
            black_box(bytes);
        });
    });
    ...
    group.bench_function("medium", |b| {
        ...
        b.iter(|| {
            let bytes = TrieBuilder::build(black_box(&refs))
                .expect("Failed to build");
            black_box(bytes);
        });
    });
    ...
    for size in [100, 1000, 5000].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                ...
                // 압축률 측정을 위한 원본 크기 계산
                let original_size: usize = entries.iter()
