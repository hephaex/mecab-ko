...
impl SearchEngine {
    ...
    /// 문서 인덱싱
    fn index_document(&self, doc: &Document) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// - 복합명사 분해
    /// - 동의어 확장
    fn tokenize_for_indexing(&self, text: &str) -> Result<Vec<Token>, Box<dyn std::error::Error>> {
    ...
    /// - 어간 추출
    /// - 정규화 적용
    fn tokenize_for_query(&self, query: &str) -> Result<Vec<Token>, Box<dyn std::error::Error>> {
        let mut tokenizer = self.tokenizer.clone();
    ...
    /// 기본 검색
    fn search(&self, query: &str) -> Result<Vec<u64>, Box<dyn std::error::Error>> {
    ...
    /// 구문 검색 (Phrase Search)
    fn phrase_search(&self, phrase: &str) -> Result<Vec<u64>, Box<dyn std::error::Error>> {
    ...
    text.chars()
        .filter_map(|ch| {
            if Hangul::is_syllable(ch) {
                Hangul::decompose(ch).map(|(cho, _, _)| Hangul::compose_jamo(cho, 'ㅏ', ' ').unwrap_or(ch))
                    .and_then(|composed| Hangul::decompose(composed))
                    .map(|(cho, _, _)| cho)
            } else {
                None
            }
