...
impl PreprocessingPipeline {
    ...
    /// 전체 전처리 실행
    fn process(&self, text: &str) -> Result<ProcessedDocument, Box<dyn std::error::Error>> {
    ...
    /// 토큰화
    fn tokenize(&self, text: &str) -> Result<Vec<Token>, Box<dyn std::error::Error>> {
        let mut tokenizer = self.tokenizer.clone();
        Ok(tokenizer.tokenize(text))
    }

    /// 명사 추출
    fn extract_nouns(&self, text: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        let mut tokenizer = self.tokenizer.clone();
    ...
    /// 불용어 제거
    fn remove_stopwords(&self, text: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        let mut tokenizer = self.tokenizer.clone();
        let tokens = tokenizer.tokenize(text);
    ...
    /// 어간 추출 (기본형)
    fn extract_lemmas(&self, text: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        let mut tokenizer = self.tokenizer.clone();
        let tokens = tokenizer.tokenize(text);
    ...
    fn extract_features_for_classification(
        ...
        let mut tokenizer = self.tokenizer.clone();
        let tokens = tokenizer.tokenize(&self.normalize_text(text));
