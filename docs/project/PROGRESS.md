...
- **현재 정확도**: **79.1%** (세종 모드, sample.tsv 기준)
- **문장 정확도**: 39.7%
- **목표**: 85%+
...
### Sprint 37 핵심 개선
1. **Token Accuracy**: 77.0% → **79.1%** (+2.1%)
2. **Jamo 정규화**: 종성 자모(U+11xx)를 호환 자모(U+31xx)로 통일 (124차)
3. **VA 정확도**: 65.5% → **75.9%** (+10.4%) 🎉
4. **EF 정확도**: 48.4% → **55.0%** (+6.6%)
5. **EC 정확도**: 26.5% → **32.5%** (+6.0%)
6. **VX 정확도**: 37.1% → **40.0%** (+2.9%)
7. **ㄹ까요 EF 규칙**: 문장 끝에서 EC→EF 변환 (125차)
8. **보조동사 EF→EC**: "먹어 버렸다" 패턴 보정 (126차)
9. **형용사 기본형 NNP→VA**: "크다/NNP" → "크/VA 다/EF" (127차)
10. **연결어미 EF→EC**: 아서/어서/니까 등 문장 중간 EC 변환 (128차)
