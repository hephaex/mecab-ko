...
- **현재 정확도**: **78.1%** (세종 모드, sample.tsv 기준)
- **문장 정확도**: 38.3%
- **목표**: 85%+
...
| MAG  | 38.7%  | 31      | - |
| ETM  | 35.5%  | 31      | - |
| EC   | 29.1%  | 117     | +2.6% (126차) |

### Sprint 37 핵심 개선
1. **Token Accuracy**: 77.0% → **78.1%** (+1.1%)
2. **Jamo 정규화**: 종성 자모(U+11xx)를 호환 자모(U+31xx)로 통일 (124차)
3. **EF 정확도**: 48.4% → **52.3%** (+3.9%)
4. **EC 정확도**: 26.5% → **29.1%** (+2.6%)
5. **VX 정확도**: 37.1% → **40.0%** (+2.9%)
6. **ㄹ까요 EF 규칙**: 문장 끝에서 EC→EF 변환 (125차)
7. **보조동사 EF→EC**: "먹어 버렸다" 패턴 보정 (126차)
