...
## 마지막 업데이트: 2026-03-13 (Sprint 37 진행 중)

### 🎯 Sprint 37 - 85% 정확도 목표 (완료 ✅)
- **최종 정확도**: **81.7%** (세종 모드, sample.tsv 기준) 🎉
- **문장 정확도**: 51.3%
- **목표**: 85%+ (후처리 한계 도달)

#### 한계 분석
85% 달성을 위해 MeCab 사전 개선을 시도했으나, 다음 이유로 한계에 도달:
1. **연결 비용 행렬**: 사용자 사전 비용을 낮춰도 연결 비용이 높으면 선택 안 됨
2. **토큰 경계**: MeCab Viterbi 알고리즘의 토큰 분리 결정은 후처리로 복구 불가
3. **복합어 분석**: "올해", "산책" 등이 잘못 분리되는 문제
4. **불규칙 활용**: 사전에 없는 활용형은 추론 불가

**권장 다음 단계**: 딥러닝 기반 POS 태거 또는 MeCab 연결 비용 행렬 재학습
...
| NP   | 80.8%  | 26      | - |
| JKO  | 76.9%  | 52      | - |
| VA   | 75.9%  | 87      | +10.4% (127차) |
| JKS  | 71.2%  | 52      | - |
| NNG  | 71.1%  | 357     | +0.8% (129차) |
| XSV  | 59.4%  | 64      | - |
| EP   | 58.4%  | 89      | +2.2% (133차) |
| EF   | 58.5%  | 258     | +6.2% (127-134차) |
| VV   | 54.4%  | 226     | +4.0% (130-134차) |
| NNB  | 52.2%  | 23      | - |
| MAG  | 45.2%  | 31      | +6.5% (137차) |
| VX   | 40.0%  | 35      | +2.9% (126차) |
| ETM  | 35.5%  | 31      | - |
| EC   | 32.5%  | 117     | +3.4% (128차) |

### Sprint 37 핵심 개선
1. **Token Accuracy**: 77.0% → **81.7%** (+4.7%) 🎉
2. **Sentence Accuracy**: 38.0% → **52.0%** (+14.0%) 🎉
3. **Jamo 정규화**: 종성 자모(U+11xx)를 호환 자모(U+31xx)로 통일 (124차)
4. **VA 정확도**: 65.5% → **75.9%** (+10.4%) 🎉
5. **NNG 정확도**: 70.3% → **71.1%** (+0.8%)
6. **EF 정확도**: 48.4% → **58.5%** (+10.1%)
7. **EC 정확도**: 26.5% → **32.5%** (+6.0%)
8. **VX 정확도**: 37.1% → **40.0%** (+2.9%)
9. **VV 정확도**: 50.4% → **54.4%** (+4.0%)
10. **EP 정확도**: 56.2% → **58.4%** (+2.2%)
11. **MAG 정확도**: 38.7% → **45.2%** (+6.5%) (137차)
