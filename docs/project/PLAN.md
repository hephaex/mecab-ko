# 현재 스프린트: Phase 24 - Sprint 36 (EC/VX 정확도 개선)

## 목표
Token Accuracy 75.5% → 80%+ 달성
- EC (연결어미) 23.1% → 50%+
- VX (보조용언) 20.0% → 50%+
- ETM (관형형어미) 35.5% → 50%+

## 현재 정확도: 75.5% (Sample 데이터셋 기준, 2026-03-12)
...
### P0 (Critical)
- [ ] S36-01: VX (보조용언) 정확도 개선
  - 현재 20.0% → 목표 50%+
  - 패턴: VV/VA + 고/아/어 + VX (싶다, 있다, 보다, 주다)
  - sejong.rs: VX 보정 규칙 추가
  - 사용자 사전: VX 활용형 추가

- [ ] S36-02: EC (연결어미) 정확도 개선
  - 현재 23.1% → 목표 50%+
  - 패턴: VV + 고/아서/면서/지만/으면/니까 등
  - sejong.rs: EC 분리 규칙 강화
  - 복합 EC 병합 (으면서, 지만은 등)

### P1 (High)
- [ ] S36-03: ETM (관형형어미) 정확도 개선
  - 현재 35.5% → 목표 50%+
  - 패턴: VV + ㄴ/ㄹ/는/던 등
  - 종성 분리 로직 개선

- [ ] S36-04: MAG (부사) 정확도 개선
  - 현재 38.7% → 목표 60%+
  - 사용자 사전에 고빈도 부사 추가
  - NNG → MAG 변환 규칙

### P2 (Medium)
- [ ] S36-05: EF (종결어미) 정확도 개선
  - 현재 45.7% → 목표 60%+
  - 문장 끝 EC → EF 변환 강화
...
### P3 (Low)
- [ ] S36-07: 정확도 평가 CI 자동화
- [ ] S36-08: ONNX Runtime 통합 준비
