...
- [ ] S23-02: 정확도 45% 달성 (진행 중)
  ...
  - **현재 정확도**: 24.2% (22.9% → 23.4% → 24.2%, +1.3%p)
  - **문장 완전 일치**: 13 / 160 (8.1%)
  - **품사별 개선**:
    - JKO(목적격조사): 0% → 36.4% (+36.4%p)
  - **주요 남은 이슈**:
    - 사전 레벨 오분석: "친구와" → "친/VV+ETM 구와/NNG" (Viterbi 경로 문제)
    - 사전 레벨 오분석: "날씨가" → "날/NNG 씨/EP 가/EF"
    - Viterbi 경로 선택 문제: 연결 비용(connection cost)이 잘못된 경로 선호
    - 사전 품질 문제로 후처리만으로는 45% 달성 어려움
  - **다음 작업**:
    - 사전 연결 비용 조정 (dict-builder)
    - 패턴 기반 토큰 병합 후처리 추가
