# 현재 스프린트: Phase 22 - Sprint 33 (EC/ETM 정확도 개선)

## 목표
EC (연결어미) 31.6% → 50%+, ETM (관형사형어미) 25.8% → 40%+ 개선

## 현재 정확도: 59.2% (Sample 데이터셋 기준, 사용자 사전 포함)

## Sprint 33 작업 목록

### P0 (Critical) ✅
- [x] S33-01: EC (연결어미) 정확도 개선 ✅
  - 31.6% → **51.3%** (+19.7%) 🎉
  - sejong.rs: 45~48차 보정 추가
    - 45차: 연속된 EC 병합 (면서, 아서, 니까 등)
    - 46차: "으/IC + 면/EC" → "으면/EC" 병합
    - 47차: "으/EF + 면서/EC" → "으면서/EC" 병합
    - 48차: "VV(으로 끝남) + 면/EC" → "VV + 으면/EC"
  - verb-inflections.csv: VV+EC, VA+EC 패턴 추가

- [x] S33-02: ETM (관형사형어미) 정확도 개선 ✅
  - 25.8% → **48.4%** (+22.6%) 🎉
  - sejong.rs: 40.5차 보정 추가
    - 단일 음절 VV → VV + ㄴ/ㄹ/ETM 분리
    - "간" → "가/VV ㄴ/ETM", "할" → "하/VV ㄹ/ETM" 등
  - verb-inflections.csv: VV+ETM 패턴 추가 (간, 온, 먹을 등)
  - 56차 보정: ETM 유니코드 정규화 (ᆫ→ㄴ, ᆯ→ㄹ, ᆷ→ㅁ)

### P1 (High)
- [ ] S33-03: 정확도 60% 달성 목표 (진행 중)
  - 56.6% → **59.2%** (+2.6%)
  - 목표 60%까지 **0.8%** 추가 개선 필요
  - 추가 개선 사항 (2026-03-10):
    - 51차 보정: VV "신" + NNG → XPN + NNG (신제품, 신기술)
    - 52차 보정: VV "세요" 패턴 분리 (가세요 → 가/VV + 시/EP + 어요/EF)
    - 53차 보정: MM/VA → XPN 변환 (새, 첫, 맨, 헛, 옛, 순, 큰, 작)
    - 53차 보정: VV+EF "ㄹ게요/ㄹ까요/ㄹ래요" 분리
    - 54차 보정: VX "있" → VV (보조동사 아닌 경우)
    - 55차 보정: EC 뒤 "하/VV ㅂ니다/EF" → "합니다/EF" 병합
    - 56차 보정: ETM 유니코드 정규화
    - remove_jongseong_rieul() 함수 추가
    - remove_jongseong_bieup() 함수 추가
  - 저정확도 품사 분석:
    - NNG: 64.4% (개선 여지 있음 - 동음이의어)
    - VV: 58.0% (개선됨)
    - EF: 53.9% (개선됨)
    - ETM: 48.4% (개선됨)

- [ ] S33-04: 사용자 사전을 시스템 사전에 통합
  - verb-inflections.csv (373개) → Inflect.csv 병합
  - 별도 로딩 없이 기본 정확도 향상

### P2 (Medium)
- [ ] S33-05: ONNX Runtime 통합 준비
  - Cargo.toml에 ort (ONNX Runtime) 의존성 추가
  - 기본 추론 파이프라인 스텁

- [ ] S33-06: 정확도 평가 CI 자동화
  - accuracy_eval.rs 테스트를 CI에서 실행
  - 품사별 정확도 리포트 자동 생성

### P3 (Low)
- [~] S33-07: PyPI 계정 복구 (응답 대기)
- [ ] S33-08: Issue #11 (Outdated dependencies) 처리

---

# 완료된 스프린트: Phase 21 - Sprint 32 (사전 통합 + 정확도 검증) ✅ 🎉

## 목표 (100% 달성!)
생성된 사전을 MeCab에 통합하고 정확도 55%+ 달성 ✅

## 최종 정확도: **56.6%** (목표 55% 초과!)

## Sprint 32 작업 목록 (완료)

### P0 (Critical) ✅
- [x] S32-01: Tokenizer 버그 수정 ✅
  - `common_prefix_search` 대신 `get_entry` 사용
  - 정확도 0.3% → 44.8% 대폭 향상 (커밋: `21da228`)

- [x] S32-02: 생성된 사전을 MeCab-Ko-Dic에 통합 ✅
  - Inflect.csv: 56,160개 (기존 44,850 + 신규 11,310)
  - 커밋: `d1c9652`

### P1 (High) ✅
- [x] S32-03: VX 패턴 정확도 개선 ✅ (5.7% → 57.1%, +51.4%)
- [x] S32-04: EF 패턴 정확도 개선 ✅ (35.3% → 50.8%, +15.5%)
- [x] S32-05: EP 패턴 정확도 개선 ✅ (30.3% → 57.3%, +27.0%)

### P2 (Medium) ✅
- [x] S32-06: 정확도 55% 달성 목표 ✅ **56.6% 달성!**
  - 43.9% → 56.6% (+12.7%)
  - 주요 개선:
    - VX+합니다 패턴 중복 토큰 제거
...
### 성과
- Token Accuracy: 56.6% (목표 55%+ 달성!)
- Sentence Accuracy: 35.3%
- 완전 일치: 106/300 문장
- 품사별 큰 개선: VX +51.4%, JKO +40.4%, NNG +22.1%

---

# 완료된 스프린트: Phase 20 - Sprint 31 (MeCab 사전 개선) ✅

## 목표 (94.7% 달성)
MeCab 사전 레벨 개선을 위한 활용형 자동 생성기 구현 ✅

## Sprint 31 작업 목록 (완료)

### P0 (Critical) ✅
- [x] S31-01: 활용형 자동 생성기 구현 ✅
  - inflect_gen.rs 모듈 (6종 불규칙, 모음조화)
- [x] S31-02: Inflect.csv 확장 ✅
  - 10,260개 활용형 생성

### P1 (High) ✅
- [x] S31-03: ETM 사전 확장 ✅ (570개)
- [x] S31-04: 불규칙 동사 패턴 완성 ✅

### P2 (Medium) ✅
- [x] S31-05: JKS 사전 확장 ✅ (1,050개)
- [x] S31-06: EC 사전 확장 ✅ (25+ 패턴)
- [x] S31-07: 세종 코퍼스 빈도 분석 ✅

### P3 (Low)
- [~] S31-08: PyPI 계정 복구 (응답 대기)
- [x] S31-09: BERT 기반 재순위화 조사 ✅
  - docs/research/bert-reranking.md

### 성과
- 총 11,310개 사전 엔트리 생성
- BERT reranker 기술 조사 완료

---

# 완료된 스프린트: Phase 19 - Sprint 30 (정확도 개선 + npm 배포) ✅

## 목표 (85% 달성)
정확도 55% 달성 → **52.9% (코드 레벨 최대치 도달)**, npm v0.4.0 배포 ✅

## Sprint 30 작업 목록 (완료)

### P0 (Critical)
- [~] S30-01: 정확도 55% 달성 → **52.9%** (BLOCKED: MeCab 사전 한계)
  - 시작: 50.1% → 최종: 52.9% (+2.8%)
  - Sentence: 30.3% → 36.0% (+5.7%)
  - 40차 보정 패턴 적용 완료
  - 숫자/감탄사 사용자 사전 추가 시도 → 동음이의어 충돌로 롤백
- [x] S30-02: npm mecab-ko-wasm v0.4.0 배포 ✅
  - URL: https://www.npmjs.com/package/mecab-ko-wasm
  - 새 npm 토큰 생성 (Bypass 2FA)
  - GitHub Secret 업데이트 완료

### P1 (High)
- [~] S30-03: ETM 정확도 개선 → BLOCKED (사전 한계)
- [~] S30-04: JKS 정확도 개선 → BLOCKED (사전 한계)

### P2 (Medium)
- [~] S30-05: EC 정확도 개선 → BLOCKED (사전 한계)
- [~] S30-06: 숫자/시간 표현 처리 → BLOCKED (동음이의어 충돌)

### P3 (Low)
- [~] S30-07: PyPI 배포 → 응답 대기 (Issue #9540)
- [~] S30-08: GitHub Contributors "claude" 제거 → Discussion 응답 대기

### 학습 포인트
1. npm Granular Access Token은 "Bypass 2FA" 옵션 필요
2. 사용자 사전으로 단일 품사 강제 할당 시 동음이의어 충돌 발생
3. 코드 레벨 후처리의 한계 (52.9%) → 사전 자체 개선 필요

---

# 완료된 스프린트: Phase 18 - Sprint 29 (정확도 50% 달성) ✅
...
### P0 (Critical) ✅
- [x] S29-01: 정확도 50% 달성 ✅ 🎉
  - 시작: 45.0% → 최종: **50.1%** (+5.1%p)
  - VV+VX+EF 피동/사동 분리 (보이다→보/VV+이/VX+다/EF)
  - JKS 명사 사전 추가 (20개)
  - "들이" 분해 버그 수정
  - "올해" 복합어 추가
  - "나의/우리의" 대명사 분리 (17차 보정)
- [→] S29-02: npm mecab-ko-wasm v0.4.0 배포 → Sprint 30 이월

### P1 (High) ✅
- [x] S29-03: JKS (주격조사) 정확도 개선 ✅
  - 25.0% → 28.8% (+3.8%p)
- [x] S29-04: VX (보조용언) 정확도 개선 ✅
  - 31.4% → 51.4% (+20.0%p) - 대폭 개선!

### P2 (Medium)
- [x] S29-05: EC (연결어미) 정확도 - 37.6% (유지)
- [x] S29-06: ETM (관형형어미) 정확도 - 25.8% (유지)

### P3 (Low)
- [→] S29-07: PyPI 실제 배포 → Sprint 30 이월
- [→] S29-08: 문서 사이트 v0.5.0 갱신 → Sprint 30 이월
