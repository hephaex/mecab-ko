# 현재 스프린트: Phase 21 - Sprint 32 (사전 통합 + 정확도 검증)

## 목표
생성된 사전을 MeCab에 통합하고 정확도 55%+ 달성 검증

## 현재 정확도: 44.8% (Sample 데이터셋 기준)
...
- [x] S32-02: 생성된 사전을 MeCab-Ko-Dic에 통합 ✅
  - generated-inflections.csv (10,260개) → Inflect.csv에 병합
  - generated-particles.csv (1,050개) → Inflect.csv에 병합
  - 총 Inflect.csv: 56,160개 (기존 44,850 + 신규 11,310)
  - 커밋: `d1c9652`

### P1 (High)
- [ ] S32-03: EC (연결어미) 정확도 개선 (현재 14.5%)
  - 연결어미 분리 규칙 세분화
  - 고/아서/면/으면서 등 패턴 보완

- [ ] S32-04: ETM (관형사형어미) 정확도 개선 (현재 6.5%)
  - 관형사형 어미 분리 규칙 보완
  - VV+는/ㄴ/ㄹ 패턴 분석
...
### P2 (Medium)
- [ ] S32-05: ONNX Runtime 통합 준비
  - Cargo.toml에 ort (ONNX Runtime) 의존성 추가
  - 기본 추론 파이프라인 스텁

- [ ] S32-06: DistilKoBERT 모델 변환
  - Hugging Face → ONNX 변환
  - 모델 크기/성능 테스트

### P3 (Low)
- [~] S32-07: PyPI 계정 복구 (응답 대기)
- [ ] S32-08: Issue #11 (Outdated dependencies) 처리
